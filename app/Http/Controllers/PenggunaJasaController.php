...
class PenggunaJasaController extends Controller
{
    ...
    public function edit($id)
    {
        $penomoran = Penomoran::where('id', $id)
            ->where('pengguna_jasa_id', auth()->id())
            ->with(['pengirim', 'penerima', 'pemberitahu', 'suratIzin', 'pengangkutan', 'pib', 'uraianBarang'])
            ->firstOrFail();

        return view('pengguna_jasa.pengajuan.create', compact('penomoran'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        DB::transaction(function () use ($validated, &$penomoran) {
            $penomoran->status_pengajuan = 'pending_staff';
            $penomoran->submitted_by_pengguna_at = now();
            $penomoran->save();
        ...
        return redirect()->route('pengguna-jasa.pengajuan.show', $penomoran->id)
            ->with(['success' => 'Pengajuan berhasil diperbarui dan siap untuk finalisasi.']);
    }
}
